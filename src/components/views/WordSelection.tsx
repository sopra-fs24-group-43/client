...
const WordSelection = ({ isOpen, onClose, time, isDrawer, sendWordChoice, threeWords }) => {
  ...
  if (!isDrawer) {
    return (
      <div className={`wordSelection${localStorage.getItem("isDarkMode") ? "_dark" : ""} container ${isOpen ? 'open' : ''}`}>
        <div className={`wordSelection${localStorage.getItem("isDarkMode") ? "_dark" : ""} modal-content`}>
          <div className={`wordSelection${localStorage.getItem("isDarkMode") ? "_dark" : ""} title`}>
  ...
  if(isDrawer) {
    return (
      <div className={`wordSelection${localStorage.getItem("isDarkMode") ? "_dark" : ""} ${isOpen ? 'open' : ''}`}>
        <div className={`wordSelection${localStorage.getItem("isDarkMode") ? "_dark" : ""} modal-content`}>
          <div className={`wordSelection${localStorage.getItem("isDarkMode") ? "_dark" : ""} title`}>
            Select a word to draw! {time}
          </div>
          <div className={`wordSelection${localStorage.getItem("isDarkMode") ? "_dark" : ""} words-container`}>
            <div className={`wordSelection${localStorage.getItem("isDarkMode") ? "_dark" : ""} words`}>
